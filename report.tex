...
\begin{itemize}
    ...
    \item \textit{Directory Traversal Detection.} Rapid \texttt{getdents64} calls 
    (directory listing) combined with concurrent write activity trigger a 
    ``scan-then-encrypt'' alert. Directory scanning alone (e.g., \texttt{find}) does 
    not trigger without accompanying writes. The conjunctive condition is important: 
    backup tools and indexers frequently perform directory traversal, so traversal 
    alone would produce an unacceptably high false-positive rate.
    ...
    % However, 
    % canary effectiveness depends on ransomware traversing monitored directories 
    % before encrypting them --- adversaries aware of this technique could enumerate 
    % and skip canary files. The implementation wires canary directories and whitelist 
    % configuration through the monitor entry point, so these controls are reachable 
    % in ordinary end-to-end tests when configured.
\end{itemize}
...
\begin{enumerate}
    ...
    A configurable set of trusted process names (\texttt{gcc}, \texttt{dd}, 
    \texttt{ffmpeg}, \texttt{gzip}, \texttt{gpg}, etc.) is skipped during analysis. 
    To prevent adversaries from abusing whitelisted names (e.g., by naming a malicious 
    binary \texttt{gcc}), the whitelist is hardened with two checks: 
...
\begin{enumerate}
    ...
    \item \textit{Quarantine.} Since step~1 has already killed the process, the binary is no longer memory-mapped and can be safely \emph{moved} (not copied) from its original location to a quarantine directory (\texttt{/var/lib/ransomware-monitor/quarantine/} by default). All permission bits are stripped from the quarantined copy (\texttt{chmod 000}), and the original path is added to a persistent blocklist. Because the original is removed from disk, no separate binary-hardening step is needed.
