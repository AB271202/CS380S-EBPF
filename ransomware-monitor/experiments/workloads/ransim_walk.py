...
def encrypt_tree() -> int:
    ...
    for root, _, filenames in os.walk(".", topdown=True):
        for filename in list(filenames):
            ...
            dest = src.with_name(src.name + ".encrypted")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Directory-walk ransomware behavior simulator")
    parser.parse_args()

    set_comm(b"ransim_walk")
    build_tree()
    encrypted = encrypt_tree()
    print(f"ransim_walk encrypted {encrypted} files")
    return 0
