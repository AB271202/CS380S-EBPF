...
We use eBPF kprobes to intercept critical system calls and kernel functions related to file I/O and process management. Specifically, we hook into:
\begin{itemize}
    ...
    \item \texttt{do\_renameat2}: To detect mass renaming of files.
    \item \texttt{do\_unlinkat}: To monitor high-frequency file deletions, a common post-encryption behavior.
    \item \texttt{iterate\_dir}: To detect rapid directory traversal (\texttt{getdents64}), a signature of ransomware scanning for targets before encryption.
    \item \texttt{do\_send\_sig\_info}: To intercept \texttt{SIGKILL} and \texttt{SIGTERM} delivery, capturing the target process name. Ransomware commonly kills antivirus, backup, and database processes before encrypting.
\end{itemize}
...
\begin{enumerate}
    ...
    We detect when a process writes high-entropy data back to a file it previously opened (in-place overwrite) and when write buffers lack recognized file-type headers (magic bytes destroyed). The eBPF layer now emits ordinary open events as well as creations, so these checks can observe true in-place modification paths rather than depending only on \texttt{O\_CREAT}. Both checks still require the process to have overwritten \emph{at least two distinct files}. This is the critical insight that separates legitimate single-file encryption (\texttt{ccencrypt} on one file) from ransomware (encrypting many files in-place).
