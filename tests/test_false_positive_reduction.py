...
class TestProcessWhitelist(unittest.TestCase):
    ...
    def test_whitelisted_gpg_write_generates_no_alert(self):
        ...
        high_entropy_buf = os.urandom(128)
        evt = make_event(1, 1004, "gpg", "/home/user/secret.txt.gpg", 128, high_entropy_buf)
        self.detector.analyze_event(evt)
        self.assertEqual(len(self.detector.alerts), 0)
    ...
    def test_whitelisted_process_suspicious_extension_no_alert(self):
        evt = make_event(0, 1002, "rsync", "/backup/file.locked", 0, b"")
        self.detector.analyze_event(evt)
        self.assertEqual(len(self.detector.alerts), 0)

    def test_whitelisted_process_rename_no_alert(self):
        evt = make_event(2, 1003, "git", "/repo/file.crypto", 0, b"")
        self.detector.analyze_event(evt)
        self.assertEqual(len(self.detector.alerts), 0)

    def test_non_whitelisted_process_still_triggers_alert(self):
        evt = make_event(0, 2000, "evil", "/home/user/file.locked", 0, b"")
        self.detector.analyze_event(evt)
...
class TestCanaryFiles(unittest.TestCase):
    ...
    def test_canary_access_triggers_critical_alert(self):
        ...
        evt = make_event(0, 3000, "evil", canary, 0, b"")
        self.detector.analyze_event(evt)
        self.assertEqual(len(self.detector.alerts), 1)
    ...
    def test_canary_access_by_whitelisted_process_still_alerts(self):
        ...
        evt = make_event(1, 3001, "git", canary, 64, os.urandom(64))
        self.detector.analyze_event(evt)
        self.assertEqual(len(self.detector.alerts), 1)
        self.assertEqual(self.detector.alerts[0]["severity"], "critical")
    ...
    def test_non_canary_file_is_not_flagged_as_canary(self):
        ...
        with open(normal, "w") as fh:
            fh.write("hello")
        evt = make_event(0, 3002, "evil", normal, 0, b"")
...
class TestWriteTargetClassification(unittest.TestCase):
    ...
    def test_var_log_is_user_file(self):
        """Log files are real data — ransomware can target them."""
        self.assertTrue(RansomwareDetector.is_user_file("/var/log/syslog"))

    def test_var_lib_is_user_file(self):
        """Database state is real data — ransomware can target it."""
        self.assertTrue(RansomwareDetector.is_user_file("/var/lib/dpkg/status"))
    ...
    def test_system_path_writes_do_not_trigger_alerts(self):
        ...
        buf = os.urandom(128)
        for i in range(20):
            evt = make_event(1, 11000, "defrag", f"/dev/sda", 128, buf)
            det.analyze_event(evt)
        self.assertEqual(len(det.alerts), 0)

    def test_var_lib_writes_whitelisted_no_alerts(self):
        """Whitelisted database writing to /var/lib/ → no alerts."""
        det = RansomwareDetector(
            threshold_writes=3, time_window=10.0,
            verify_binary_hash=False, verify_lineage=False,
        )
        buf = os.urandom(128)
        for i in range(20):
            evt = make_event(1, 11001, "mysqld", f"/var/lib/mysql/db_{i}.ibd", 128, buf)
            det.analyze_event(evt)
        self.assertEqual(len(det.alerts), 0)
    ...
    def test_user_path_writes_still_trigger_alerts(self):
        ...
        buf = os.urandom(128)
        for i in range(10):
            evt = make_event(1, 11002, "evil", f"/home/user/file_{i}.doc", 128, buf)
            det.analyze_event(evt)
        relevant = [
            a for a in det.alerts
            if a["reason"] in ("Magic bytes destroyed", "High entropy + Frequency",
    ...
    def test_var_lib_writes_from_unknown_process_trigger_alerts(self):
        """Non-whitelisted process encrypting /var/lib/ database files → alerts."""
        det = RansomwareDetector(
            threshold_writes=3, time_window=10.0,
            verify_binary_hash=False, verify_lineage=False,
        )
        buf = os.urandom(128)
        for i in range(10):
            evt = make_event(1, 11003, "evil", f"/var/lib/postgresql/data/{i}.dat", 128, buf)
            det.analyze_event(evt)
        relevant = [
            a for a in det.alerts
            if a["reason"] in ("Magic bytes destroyed", "High entropy + Frequency",
...
class TestFileDiversityScoring(unittest.TestCase):
    ...
    def test_low_entropy_diverse_writes_no_alert(self):
        ...
        for d in dirs:
            for j in range(3):
                evt = make_event(1, 12002, "writer", f"{d}/f{j}.txt", 128, buf)
                det.analyze_event(evt)
        diversity_alerts = [
            a for a in det.alerts if a["reason"] == "High file diversity + Entropy"
        ]
    ...
    def test_defrag_same_files_no_diversity_alert(self):
        ...
        for i in range(50):
            evt = make_event(1, 12003, "defrag", "/home/user/bigfile.dat", 128, buf)
            det.analyze_event(evt)
        diversity_alerts = [
            a for a in det.alerts if a["reason"] == "High file diversity + Entropy"
        ]
...
class TestDefragVsRansomware(unittest.TestCase):
    ...
    def test_defrag_block_device_writes_no_alerts(self):
        ...
        buf = os.urandom(128)
        for i in range(50):
            evt = make_event(1, 14000, "e4defrag", "/dev/sda1", 128, buf)
            det.analyze_event(evt)
        self.assertEqual(len(det.alerts), 0)
    ...
    def test_defrag_single_file_repeated_writes_no_alerts(self):
        ...
        buf = os.urandom(128)
        for i in range(50):
            evt = make_event(1, 14001, "defrag", "/home/user/largefile.img", 128, buf)
            det.analyze_event(evt)
        diversity_alerts = [
            a for a in det.alerts if a["reason"] == "High file diversity + Entropy"
        ]
    ...
    def test_ransomware_multi_dir_encryption_detected(self):
        ...
        for f in targets:
            evt = make_event(1, 14002, "cryptolocker", f, 128, buf)
            det.analyze_event(evt)
        diversity_alerts = [
            a for a in det.alerts if a["reason"] == "High file diversity + Entropy"
        ]
...
class TestInPlaceOverwriteDetection(unittest.TestCase):
    ...
    def test_overwrite_opened_file_triggers_alert(self):
        ...
        for fname in ["/home/user/photo.jpg", "/home/user/report.docx"]:
            evt = make_event(0, 15000, "evil", fname, 0, b"")
            det.analyze_event(evt)
            evt = make_event(1, 15000, "evil", fname, 128, buf)
            det.analyze_event(evt)
        overwrite_alerts = [
    ...
    def test_single_file_in_place_encrypt_no_alert(self):
        ...
        det = self._det()
        evt = make_event(0, 15004, "ccencrypt", "/home/user/notes.bin", 0, b"")
        det.analyze_event(evt)
        buf = os.urandom(128)
        evt = make_event(1, 15004, "ccencrypt", "/home/user/notes.bin", 128, buf)
        det.analyze_event(evt)
        overwrite_alerts = [
            a for a in det.alerts if a["reason"] == "In-place overwrite"
        ]
    ...
    def test_write_to_new_file_no_overwrite_alert(self):
        ...
        det = self._det()
        buf = os.urandom(128)
        evt = make_event(1, 15001, "evil", "/home/user/newfile.enc", 128, buf)
        det.analyze_event(evt)
        overwrite_alerts = [
            a for a in det.alerts if a["reason"] == "In-place overwrite"
        ]
    ...
    def test_low_entropy_overwrite_no_alert(self):
        ...
        det = self._det()
        evt = make_event(0, 15002, "editor", "/home/user/notes.txt", 0, b"")
        det.analyze_event(evt)
        buf = b"A" * 128
        evt = make_event(1, 15002, "editor", "/home/user/notes.txt", 128, buf)
        det.analyze_event(evt)
        overwrite_alerts = [
            a for a in det.alerts if a["reason"] == "In-place overwrite"
        ]
    ...
    def test_legitimate_gz_output_no_overwrite_alert(self):
        ...
        det = self._det()
        # Open the source
        evt = make_event(0, 15003, "gzip_sim", "/home/user/report.txt", 0, b"")
        det.analyze_event(evt)
        # Write compressed output to a .gz derivative
        buf = os.urandom(128)
        evt = make_event(1, 15003, "gzip_sim", "/home/user/report.txt.gz", 128, buf)
        det.analyze_event(evt)
        overwrite_alerts = [
            a for a in det.alerts if a["reason"] == "In-place overwrite"
        ]
...
class TestWriteThenUnlinkCorrelation(unittest.TestCase):
    ...
    def test_write_multiple_then_delete_original_triggers(self):
        ...
        for i in range(4):
            evt = make_event(1, 16000, "evil", f"/home/user/f{i}.locked", 128, buf)
            det.analyze_event(evt)
        # Delete an original (different from write targets)
        evt = make_event(3, 16000, "evil", "/home/user/original.docx", 0, b"")
        det.analyze_event(evt)
        wtu_alerts = [
    ...
    def test_delete_own_write_target_no_alert(self):
        ...
        buf = os.urandom(128)
        for i in range(4):
            evt = make_event(1, 16001, "builder", f"/tmp/out_{i}.o", 128, buf)
            det.analyze_event(evt)
        # Delete one of the files we wrote to
        evt = make_event(3, 16001, "builder", "/tmp/out_0.o", 0, b"")
        det.analyze_event(evt)
        wtu_alerts = [
            a for a in det.alerts if a["reason"] == "Write-then-delete"
        ]
    ...
    def test_single_write_then_delete_no_alert(self):
        ...
        det = self._det()
        buf = os.urandom(128)
        evt = make_event(1, 16002, "gzip_sim", "/home/user/data.csv.gz", 128, buf)
        det.analyze_event(evt)
        evt = make_event(3, 16002, "gzip_sim", "/home/user/data.csv", 0, b"")
        det.analyze_event(evt)
        wtu_alerts = [
            a for a in det.alerts if a["reason"] == "Write-then-delete"
        ]
    ...
    def test_low_entropy_writes_then_delete_no_alert(self):
        ...
        buf = b"A" * 128
        for i in range(5):
            evt = make_event(1, 16003, "writer", f"/home/user/f{i}.txt", 128, buf)
            det.analyze_event(evt)
        evt = make_event(3, 16003, "writer", "/home/user/original.txt", 0, b"")
        det.analyze_event(evt)
        wtu_alerts = [
            a for a in det.alerts if a["reason"] == "Write-then-delete"
        ]
    ...
    def test_write_targets_expire_outside_window(self):
        ...
        buf = os.urandom(128)
        for i in range(4):
            evt = make_event(1, 16004, "evil", f"/home/user/f{i}.locked", 128, buf)
            det.analyze_event(evt)
            time.sleep(0.02)
        evt = make_event(3, 16004, "evil", "/home/user/original.docx", 0, b"")
        det.analyze_event(evt)
        wtu_alerts = [
            a for a in det.alerts if a["reason"] == "Write-then-delete"
        ]
...
class TestLegitEncryptionVsRansomware(unittest.TestCase):
    ...
    def test_gzip_single_file_no_alerts(self):
        ...
        det = self._det()
        # Open source
        evt = make_event(0, 17000, "gzip_sim", "/home/user/data.csv", 0, b"")
        det.analyze_event(evt)
        # Write compressed output
        buf = os.urandom(128)
        evt = make_event(1, 17000, "gzip_sim", "/home/user/data.csv.gz", 128, buf)
        det.analyze_event(evt)
        # Delete original
        evt = make_event(3, 17000, "gzip_sim", "/home/user/data.csv", 0, b"")
        det.analyze_event(evt)
    ...
    def test_gpg_encrypt_file_no_in_place_alert(self):
        ...
        det = self._det()
        evt = make_event(0, 17001, "gpg_sim", "/home/user/secret.txt", 0, b"")
        det.analyze_event(evt)
        buf = os.urandom(128)
        evt = make_event(1, 17001, "gpg_sim", "/home/user/secret.txt.gpg", 128, buf)
        det.analyze_event(evt)
        overwrite_alerts = [
            a for a in det.alerts if a["reason"] == "In-place overwrite"
        ]
    ...
    def test_ransomware_in_place_encrypt_detected(self):
        ...
        for fname in ["/home/user/photo.jpg", "/home/user/report.docx"]:
            evt = make_event(0, 17002, "locker", fname, 0, b"")
            det.analyze_event(evt)
            evt = make_event(1, 17002, "locker", fname, 128, buf)
            det.analyze_event(evt)
        overwrite_alerts = [
            a for a in det.alerts if a["reason"] == "In-place overwrite"
        ]
    ...
    def test_ransomware_encrypt_then_delete_detected(self):
        ...
        # Write encrypted copies
        for f in originals:
            evt = make_event(1, 17003, "locker", f + ".locked", 128, buf)
            det.analyze_event(evt)
        # Delete originals
        for f in originals:
            evt = make_event(3, 17003, "locker", f, 0, b"")
            det.analyze_event(evt)
        wtu_alerts = [
            a for a in det.alerts if a["reason"] == "Write-then-delete"
        ]
    ...
    def test_zip_multiple_files_no_write_then_delete(self):
        ...
        for i in range(10):
            evt = make_event(1, 17004, "zip_sim", "/home/user/archive.zip", 128, buf)
            det.analyze_event(evt)
        wtu_alerts = [
            a for a in det.alerts if a["reason"] == "Write-then-delete"
        ]
    ...
    def test_tar_gz_pipeline_no_alerts(self):
        ...
        evt = make_event(0, 17005, "tar_sim", "/home/user/project", 0, b"")
        det.analyze_event(evt)
        buf = os.urandom(128)
        # Write to archive
        for i in range(5):
            evt = make_event(1, 17005, "tar_sim", "/home/user/project.tar.gz", 128, buf)
            det.analyze_event(evt)
        bad_alerts = [
            a for a in det.alerts
            if a["reason"] in ("In-place overwrite", "Write-then-delete")
